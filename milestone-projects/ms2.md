...
- [ ] Screenshots of all site features (Nav bar | footer | tables | etc...) accompanied by an explanation of what each feature does & link to the relevant user story.
- [ ] Thorough testing steps & screenshots of validator testing - Explain the principles of automated and manual testing and when each might be deployed.
- [ ] All deployment steps including initial steps to clone the repo (git add/commit/push | deploy to github pages)
- [ ] Credit code & media sources - Attribute any code from external sources to its source via comments above the code and (for larger dependencies) in the README.
